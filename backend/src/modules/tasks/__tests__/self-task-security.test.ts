...
const migration = readFileSync(join(root,
  "backend/supabase/migrations/202608090003_employee_self_tasks.sql"), "utf8");
const brandOptionsMigration = readFileSync(join(root,
  "backend/supabase/migrations/202608090005_employee_self_task_brand_options.sql"), "utf8");
const brandCreationMigration = readFileSync(join(root,
  "backend/supabase/migrations/202608130001_employee_self_task_brand_creation.sql"), "utf8");
const service = readFileSync(join(root, "backend/src/modules/tasks/task-service.ts"), "utf8");
const page = readFileSync(join(root, "frontend/src/app/tasks/page.tsx"), "utf8");
const component = readFileSync(join(root, "frontend/src/components/tasks/MyTasks.tsx"), "utf8");
const calendar = readFileSync(join(root, "frontend/src/components/ui/CalendarDatePill.tsx"), "utf8");
const completionMigration = readFileSync(join(root,
  "backend/supabase/migrations/202608140001_employee_complete_self_task.sql"), "utf8");
